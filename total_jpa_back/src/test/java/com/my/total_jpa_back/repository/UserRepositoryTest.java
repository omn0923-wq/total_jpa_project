...
//        이전 페이지 ?
    ...
    @Test
    @DisplayName("회원 이름 오름차순 정리")
    void orderByNameAscTest() {
//        정렬 기계를 하나 세팅
        Sort sort = Sort.by("name").ascending();
//        전체 검색할 때 소트 기계를 삽입해서 정렬되도록 처리
...
//        }
    ...
    void orderByCreatedAtDescTest() {
        ...
        users.stream().limit(10).forEach(x -> log.info("user_id: {}, name: {}, 가입일: {}",
                x.getId(), x.getName(), x.getCreatedAt()));
    }

    //    색상 오름차순, 같은 색상 자료는 이름 내림차순, 상위 100개 출력
    @Test
    @DisplayName("색상 오름차순, 같은 색상 자료는 이름 내림차순")
    void multiSortTest() {
        Sort sort = Sort.by("likeColor").ascending().and(
                Sort.by("name").descending()
        );
        List<Users> users = userRepository.findAll(sort);
        users.stream().limit(100).forEach(x -> log.info("color: {}, name: {}",
...
//        then
    ...
    @Test
    @DisplayName("이름에 Kim을 포함하는 자료 검색")
    void 킴_찾기() {
